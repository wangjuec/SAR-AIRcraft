...
def draw_annotations(image, annotations, box_color, text_color):
    ...
    for annotation in annotations:
        for obj in annotation.findall('object'):
            ...
            draw.text((xmin, ymin - 15), name, fill=text_color)
    return image

def draw_yolo_predictions(image, results, box_color, text_color):
    draw = ImageDraw.Draw(image)
    for result in results:
        boxes = result.boxes
        for i in range(len(boxes)):
            ...
            confidence = float(boxes.conf[i])
            draw.rectangle([xmin, ymin, xmax, ymax], outline=box_color, width=3)
            draw.text((xmin, ymin - 15), f"{label} {confidence:.2f}", fill=text_color)
    return image
...
if root is not None:
    ...
    image_with_annotations = draw_annotations(image_with_annotations, annotations, "red", "red")
...
# 显示YOLO预测图片
image_with_yolo = image.copy()
image_with_yolo = draw_yolo_predictions(image_with_yolo, results, "green", "green")

# 叠加对比图片
image_combined = image_with_annotations.copy()
image_combined = draw_yolo_predictions(image_combined, results, "green", "green")
...
col1.image(image, caption="原始图片")
col2.image(image_with_annotations, caption="原始标记图片")
col3.image(image_with_yolo, caption="YOLO预测图片")
col4.image(image_combined, caption="叠加对比图片")
